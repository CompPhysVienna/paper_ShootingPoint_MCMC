...
@njit
def _run_aimless_flexible_length(total_trials : int, initial_path : np.ndarray, equilibration_trials : int, 
    ...
    for trial in range(total_trials):
        ...
        if sp_index >= 0 and sp_index < current_path.shape[0]:
            ...
            if is_reactive_path:
                ...
                if np.random.random() < length_ratio and new_index >= 0 and new_index < proposal_trj.shape[0]:
...
@ray.remote
def run_aimless_flexible_length(total_trials : int, initial_path : np.ndarray, equilibration_trials : int, 
                                  state_bounds : float , dt_D : float, configuration_bins : np.ndarray, 
                                  length_bins : np.ndarray, k_bins : np.ndarray, max_dk : int):
    
    return _run_aimless_flexible_length(total_trials, initial_path, equilibration_trials, 
...
if __name__ == "__main__":
    ...
    jobs = [run_aimless_flexible_length.remote(total_trials, initial_path, equilibration_trials, 
    ...
    np.save(f"data/flexible_length/aimless/path_density.npy", trajectory_ensemble)
    np.save(f"data/flexible_length/aimless/sp_density.npy", SP_ensemble)
    np.save(f"data/flexible_length/aimless/length_histogram.npy", length_histogram)
    np.save(f"data/flexible_length/aimless/k_histogram.npy", k_histogram)
