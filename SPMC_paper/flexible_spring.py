...
@njit
def _run_spring_flexible_length(total_trials : int, initial_path : np.ndarray, equilibration_trials : int, 
    ...
    for trial in range(total_trials):
        ...
        # Select shift frome exponential
        ...
        if sp_index >= 0 and sp_index < current_path.shape[0]:
            ...
            if is_reactive_path:
                ...
                new_sp_index = proposal_index + np.arange(-max_dk, max_dk+1, 1)[np.searchsorted(np.cumsum(p), np.random.rand())]
                
                # Accept or reject
                if np.random.random() < length_ratio and new_sp_index >= 0 and new_sp_index < proposal_trj.shape[0]:
...
@ray.remote
def run_spring_flexible_length(total_trials : int, initial_path : np.ndarray, equilibration_trials : int, 
                                  state_bounds : float , dt_D : float, configuration_bins : np.ndarray, 
                                  length_bins : np.ndarray, k_bins : np.ndarray, max_dk : int, spring_strength : float):
    
    return _run_spring_flexible_length(total_trials, initial_path, equilibration_trials, 
...
if __name__ == "__main__":
    ...
    jobs = [run_spring_flexible_length.remote(total_trials, initial_path, equilibration_trials, 
    ...
    np.save(f"data/flexible_length/spring/path_density.npy", trajectory_ensemble)
    np.save(f"data/flexible_length/spring/sp_density.npy", SP_ensemble)
    np.save(f"data/flexible_length/spring/length_histogram.npy", length_histogram)
    np.save(f"data/flexible_length/spring/k_histogram.npy", k_histogram)
